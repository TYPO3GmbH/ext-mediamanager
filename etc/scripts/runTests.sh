...
# used in etc/testing-docker//localdocker-compose.yml
    ...
    case ${DBMS} in
        mysql|mariadb)
            ...
            if [ "${DATABASE_DRIVER}" != "mysqli" ] && [ "${DATABASE_DRIVER}" != "pdo_mysql" ]; then
                ...
                echo >&2
                echo "call \".etc/scripts/runTests.sh -h\" to display help and valid options" >&2
                exit 1
            fi
            ;;
        postgres|sqlite)
...
Examples:
    ...
    ./etc/scripts/runTests.sh -x

    # Run unit tests in phpunit verbose mode with xdebug on PHP 8.1 and filter for test canRetrieveValueWithGP
    ./etc/scripts/runTests.sh -x -p 8.1 -e "-v --filter canRetrieveValueWithGP"

    # Run functional tests in phpunit with a filtered test method name in a specified file
    # example will currently execute two tests, both of which start with the search term
    ./etc/scripts/runTests.sh -s functional -e "--filter deleteContent" typo3/sysext/core/Tests/Functional/DataHandling/Regular/Modify/ActionTest.php

    # Run unit tests with PHP 8.1 and have xdebug enabled
    ./etc/scripts/runTests.sh -x -p 8.1

    # Run functional tests on postgres with xdebug, php 8.1 and execute a restricted set of tests
    ./etc/scripts/runTests.sh -x -p 8.1 -s functional -d postgres typo3/sysext/core/Tests/Functional/Authentication

    # Run functional tests on mariadb 10.5
    ./etc/scripts/runTests.sh -d mariadb -i 10.5

    # Run functional tests on postgres 11
    ./etc/scripts/runTests.sh -s functional -d postgres -k 11

    # Run restricted set of application acceptance tests
    ./etc/scripts/runTests.sh -s acceptance typo3/sysext/core/Tests/Acceptance/Application/Login/BackendLoginCest.php:loginButtonMouseOver

    # Run installer tests of a new instance on sqlite
    ./etc/scripts/runTests.sh -s acceptanceInstall -d sqlite
EOF
...
if [ ${#INVALID_OPTIONS[@]} -ne 0 ]; then
    ...
    echo >&2
    echo "call \".etc/scripts/runTests.sh -h\" to display help and valid options"
    exit 1
fi
    ...
    functional)
        ...
        case ${DBMS} in
            ...
            sqlite)
                ...
                mkdir -p "${EXT_ROOT}/typo3temp/var/tests/functional-sqlite-dbs/"
